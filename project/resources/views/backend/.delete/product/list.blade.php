        ...
        <div id="kt_app_content_container" class="app-container container-fluid">
			...
			@component('backend.components.form-modal', ['modal_size' => 'modal-lg', 'modal_id' => $controller])
				@section('form_modal')
					...
					<div class="fv-row mb-7">
						<label class="required form-label fw-semibold">{{ __('main.product-type') }}</label>
						<div data-table-filter="product_type">
							<select name="product_type_id" class="form-select" data-dropdown-parent="#{{ $controller }}"  data-kt-select2="true" data-placeholder="{{ __('main.please_choose') }}" data-allow-clear="true">
					...
					<div class="fv-row mb-7">
						<label class="required fs-6 fw-semibold mb-2">{{ __('main.purchase_price') }}</label>
						<input type="number" class="form-control" placeholder="{{ __('main.purchase_price') }}" name="purchase_price" />
					</div>
					
					<div class="fv-row mb-7">
						<label class="required fs-6 fw-semibold mb-2">{{ __('main.selling_price') }}</label>
						<input type="number" class="form-control" placeholder="{{ __('main.selling_price') }}" name="selling_price" />
					</div>

					<div class="fv-row mb-7">
						<label class="required fs-6 fw-semibold mb-2">{{ __('main.stock') }}</label>
						<input type="number" class="form-control" placeholder="{{ __('main.stock') }}" name="stock" />
					</div>
...
@push('private_js')
	...
	<script>
		...
		const URL_API = `{{ url('admin/products') }}`
			...
			$.ajax({
				...
				success: function(response){
					if (response.status.code === 200) {
						...
						$('select[name="product_type_id"]').val(data.product_type_id).trigger('change').attr('data-placeholder', data.product_type_id);
						...
						if (data.assets_relative_path != null){
							...
							$("#preview_product_asset").show();
						}						
						$("#preview_product_asset").attr('src', product_asset);
						...
						{	
							...
							render: function (data, type, row) {
								...
								if (row.assets_relative_path != null){
									product_assets = path + '/' + row.assets_relative_path; 
								}
					...
					form,
					{
						fields: {
							...
							'product_type_id': {
								validators: {
									notEmpty: {
										message: `{{ __('main.product-type') }} {{ __('main.is_required') }}`
									}
								}
							},
							'stock': {
								validators: {
									notEmpty: {
										message: `{{ __('main.stock') }} {{ __('main.is_required') }}`
									}
								}
							},
							'purchase_price': {
								validators: {
									notEmpty: {
										message: `{{ __('main.purchase_price') }} {{ __('main.is_required') }}`
									}
								}
							},
							'selling_price': {
								validators: {
									notEmpty: {
										message: `{{ __('main.selling_price') }} {{ __('main.is_required') }}`
									}
								}
							}
						},
